...
var ClapprStats = function (_ContainerPlugin) {
  ...
  _createClass(ClapprStats, [{
    ...
    value: function bindEvents() {
      ...
      this.listenTo(this.container, _clappr.Events.CONTAINER_BITRATE, this.onBitrate);
      this.listenTo(this.container, _clappr.Events.CONTAINER_STOP, this.stopReporting);
      this.listenTo(this.container, _clappr.Events.CONTAINER_ENDED, this.stopReporting);
      this.listenToOnce(this.container.playback, _clappr.Events.PLAYBACK_PLAY_INTENT, this.startTimers);
      this.listenToOnce(this.container, _clappr.Events.CONTAINER_PLAY, this.onFirstPlaying);
      this.listenTo(this.container, _clappr.Events.CONTAINER_PLAY, this.onPlay);
      this.listenTo(this.container, _clappr.Events.CONTAINER_PAUSE, this.onPause);
      this.listenToOnce(this.container, _clappr.Events.CONTAINER_STATE_BUFFERING, this.onBuffering);
      this.listenTo(this.container, _clappr.Events.CONTAINER_SEEK, this.onSeek);
      this.listenTo(this.container, _clappr.Events.CONTAINER_ERROR, function () {
        return _this2._inc('error');
      });
      this.listenTo(this.container, _clappr.Events.CONTAINER_FULLSCREEN, function () {
        return _this2._inc('fullscreen');
      });
      this.listenTo(this.container, _clappr.Events.CONTAINER_PLAYBACKDVRSTATECHANGED, function (dvrInUse) {
        dvrInUse && _this2._inc('dvrUsage');
      });
      this.listenTo(this.container.playback, _clappr.Events.PLAYBACK_PROGRESS, this.onProgress);
      this.listenTo(this.container.playback, _clappr.Events.PLAYBACK_TIMEUPDATE, this.onTimeUpdate);
    }
  }, {
    key: 'onBitrate',
    ...
    value: function onFirstPlaying() {
      this.listenTo(this.container, _clappr.Events.CONTAINER_TIMEUPDATE, this.onContainerUpdateWhilePlaying);
    ...
    value: function onPause() {
      ...
      this._start('pause');
      this._inc('pause');
      this.listenToOnce(this.container, _clappr.Events.CONTAINER_PLAY, this.playAfterPause);
    }
  }, {
    key: 'onSeek',
    ...
    value: function onBuffering() {
      ...
      this.listenToOnce(this.container, _clappr.Events.CONTAINER_STATE_BUFFERFULL, this.onBufferfull);
    }
  }, {
    key: 'onBufferfull',
    ...
    value: function _calculateBitrates() {
      ...
      this._metrics.extra.bitrateWeightedMean = this._metrics.extra.bitratesHistory.map(function (x) {
        ...
        var bitrateTime = x.time || _this3._metrics.timers.watch - bitratesWatchedTime;
        return x.bitrate * bitrateTime;
      }).reduce(function (a, b) {
        return a + b;
      }, 0) / this._metrics.timers.watch;
      ...
      if (this._uriToMeasureLatency) {
        ...
        var ld = function ld() {
          ...
          if (t.length > n) done();else {
            var img = new Image();
            img.onload = ld;
            img.src = _this4._uriToMeasureLatency + '?' + Math.random() + '=' + _this4._now();
          }
        };
        var done = function done() {
          rtt = t[2] - t[1];
          _this4._metrics.timers.latency = rtt;
        };
        ld();
      }
    }
    ...
    value: function _measureBandwidth() {
      ...
      if (this._urisToMeasureBandwidth && this._bwMeasureCount % this._runBandwidthTestEvery == 0) {
        ...
        var ld = function ld(e) {
          if (i > 0) {
            _this5._urisToMeasureBandwidth[i - 1].end = _this5._now();
            clearTimeout(_this5._urisToMeasureBandwidth[i - 1].timer);
          }
          if (i >= _this5._urisToMeasureBandwidth.length || i > 0 && _this5._urisToMeasureBandwidth[i - 1].expired) done(e);else {
            var xhr = new XMLHttpRequest();
            xhr.open('GET', _this5._urisToMeasureBandwidth[i].url, true);
            xhr.responseType = 'arraybuffer';
            xhr.onload = xhr.onabort = ld;
            _this5._urisToMeasureBandwidth[i].start = _this5._now();
            _this5._urisToMeasureBandwidth[i].timer = setTimeout(function (j) {
              _this5._urisToMeasureBandwidth[j].expired = true;
              xhr.abort();
            }, _this5._urisToMeasureBandwidth[i].timeout, i);
            xhr.send();
          }
          i++;
        };
        ...
        var done = function done(e) {
          var timeSpent = (_this5._urisToMeasureBandwidth[i - 1].end - _this5._urisToMeasureBandwidth[i - 1].start) / 1000;
          var bandwidthBps = e.loaded * 8 / timeSpent;
          var bandwidthKbps = bandwidthBps / 1000;
          _this5._metrics.extra.bandwidth = bandwidthKbps;
          _this5._urisToMeasureBandwidth.forEach(function (x) {
            x.start = 0;
            x.end = 0;
            x.expired = false;
